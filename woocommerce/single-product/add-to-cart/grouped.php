					...
					switch ( $column_id ) {
							...
							if ( ! $grouped_product_child->is_purchasable() || $grouped_product_child->has_options() || ! $grouped_product_child->is_in_stock() ) {
								...
								if ( $grouped_product_child->is_on_sale() ) {
									printf(
										...
										esc_html__( 'Buy one of %1$s on sale for %2$s, original price was %3$s', 'anvogue' ),
									...
									printf(
										...
										esc_html__( 'Buy one of %1$s for %2$s', 'anvogue' ),
										esc_html( $grouped_product_child->get_name() ),
										esc_html( wp_strip_all_tags( wc_price( $grouped_product_child->get_price() ) ) )
									);
								}
						...
						case 'label':
							...
							if ( in_array( 'price', $grouped_product_columns ) ) {
								$value .= '<div class="pls-grouped-product-price">'.$grouped_product_child->get_price_html() . wc_get_stock_html( $grouped_product_child ).'</div>';
							}
							break;
						case 'price':
							continue 2;
